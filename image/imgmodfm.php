...
include '../connect.php';
$image = $_GET['image'];
$stmt = $conn->prepare("select id,alt,if(url like 'http%',url,concat(\"../images/\",url)) as murl,url from image where id=" . $image);
$stmt->execute();
$row = $stmt->fetch();
...
?></td><td><a href=ihmodfm.php?imgd=<?php echo $image ?>>edit selected headings</a></td></tr>
