...
echo "spp : " . $spp . "<br/>";
      $new_destination_id=$conn->lastInsertId();
if (isset($new_destination_id)) {
  $dst = $new_destination_id;
} else {
  $dst = $_POST['destination'];
}
echo "dst : " . $dst . "<br/>";

if ($_POST['bktyp']==1) { //FLIGHT

  $sql="insert into bk_booking(ref,carrier,cost,notes,type,booked) values ('" 
	. $_POST['booking'] . "'," 
	. $spp . "," 
	. $_POST['cost'] . ",'" 
	. $_POST['notes'] . "',"
  ...
  try {// execute the query
    ...
    // PERFORMS TWO VERY UNINTELLIGENT STAGE INSERTS, THIS WILL GO QUITE WRONG AS SOON AS SOMETHING INTRICATE IS REQUIRED
    $sql="insert into stage(booking,direction,sequence,flight,stage_date,seats) VALUES ($lstid,0,10,$outbound_flight_id_1," . $_POST['outbound_date'] . ",1)";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $sql="insert into stage(booking,direction,sequence,flight,stage_date,seats) VALUES ($lstid,1,11,$inbound_flight_id," . $_POST['inbound_date'] . ",1)";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
  } catch (PDOException $e) {
    echo $sql . "<br/>" . $e->getMessage();
  ...
  $sql="insert into bk_booking(ref,carrier,cost,notes,type,destination,booked,event_date_start,event_date_end,nights) values ('" 
	. $_POST['booking'] . "'," 
	. $spp . "," 
	. $_POST['cost'] . ",'" 
	. $_POST['notes'] . "',"
